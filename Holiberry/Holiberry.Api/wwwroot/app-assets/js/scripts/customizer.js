...
(function (window, document, $) {
  ...
  var themeColor = {
    "theme-primary":
      "linear-gradient(118deg, #ef3b2d, rgba(115, 103, 240, 0.7))",
    "theme-success":
  ...
  $(document).on("click", "#customizer-theme-colors .color-box", function () {
    ...
    if (body.data("menu") == "horizontal-menu") {
      if (
        horizontalNavbar.find("li.sidebar-group-active:not(.dropdown-submenu)")
          .length
      ) {
        horizontalNavbar
      ...
      if (menuContent.find("li.active").length) {
        ...
        $(".main-menu-content").find("li.sidebar-group-active").length
      ) {
  ...
  $("#navbar-static").on("click", function () {
    if (body.hasClass("horizontal-menu")) {
      horizontalNavbar.removeClass(
        "d-none floating-nav fixed-top navbar-fixed"
      );
      body
